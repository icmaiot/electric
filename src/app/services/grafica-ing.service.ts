...
export class GraficaIngService {
    private url: string = environment.environment.urlEndPoint + '/grafica_ing';
    constructor(private http: HttpClient) {}

    geti(token): Observable <any> {
        const headers = new HttpHeaders({ 'Content-Type': 'application/json', 'authorization': token });
        return this.http.get<any>(this.url + '/get', { headers });
    }

    getConsuli(maquina: string, r1: string, r2: string, token): Observable<any>{
    ...
    getMaqi(token): Observable <any> {
        const headers = new HttpHeaders({ 'Content-Type': 'application/json', 'Authorization': token });
        return this.http.get<any>(this.url + '/maqc', { headers });
    }

    getCausai(causa: string, maquina: string, r1: string, r2: string, token): Observable<any>{
        let params = new HttpParams();

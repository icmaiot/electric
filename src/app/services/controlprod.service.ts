...
export class ControlprodService {
  ...
  private url: string = environment.environment.urlEndPoint + '/controlprod';
  ...
  create(prod, token): Observable<any> {
    const headers = new HttpHeaders({ 'Content-Type': 'application/json', 'Authorization': token });
    return this.http.post<any>(this.url + '/get', prod, { headers });
  }
  ...
  update(obj, token): Observable<any> {
    const headers = new HttpHeaders({ 'Content-Type': 'application/json', 'Authorization': token });
    return this.http.put(`${this.url + '/read'}/${obj.idcontrolprod}`, obj, { headers });
  }

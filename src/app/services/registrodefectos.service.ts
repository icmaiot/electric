...
export class RegistrodefectosService {

  private url: string = environment.environment.urlEndPoint + '/registrodefectos';
  ...
  update(defectos, token): Observable<any> {
    const headers = new HttpHeaders({ 'Content-Type': 'application/json', 'Authorization': token });
    return this.http.put(`${this.url + '/read'}/${defectos.iddefreg}`, defectos, { headers });
  }

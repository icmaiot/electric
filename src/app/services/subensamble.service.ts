...
export class SubensambleService {

  private url: string = environment.environment.urlEndPoint + '/subensamble';
  ...
  getMaquinaBySubensamble(idsubens,token): Observable<any> {
    const headers = new HttpHeaders({ 'Content-Type': 'application/json', 'Authorization': token });
    let params = new HttpParams();
    params = params.append('idsubens', idsubens);
    return this.http.get(this.url + '/MaquinaBySubensamble', { headers, params: params });
  }
  ...
  update(obj, token): Observable<any> {
    const headers = new HttpHeaders({ 'Content-Type': 'application/json', 'Authorization': token });
    return this.http.put(`${this.url + '/read'}/${obj.idsubens}`, obj, { headers });
  }

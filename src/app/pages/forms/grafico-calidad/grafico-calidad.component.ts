...
import { GraficaCalidadService } from '@app/services/grafica-calidad.service';
...
@Component({
  selector: 'app-grafico-calidad',
  templateUrl: './grafico-calidad.component.html',
  styleUrls: ['./grafico-calidad.component.css'],
  encapsulation: ViewEncapsulation.None
})
export class GraficoCalidadComponent implements OnInit {
  form: FormGroup;
  form2: FormGroup;
  submitted = false;
  submitted2 = false;
  ...
  constructor(
   private zone: NgZone,
   private grafico_calidadService: GraficaCalidadService,
  ...
  ngOnInit() {
    ...
    this.getGraficaCalidad();
    this.getSelect();
  }

  async getGraficaCalidad(){
    try{
      let res = await this.grafico_calidadService.getc( this.auth.token ).toPromise();
      if(res.code = 200){
  ...
  async getSelect(){
    try{
      let res = await this.grafico_calidadService.getMaqc( this.auth.token ).toPromise();
      if(res.code = 200){
  ...
  onSubmits2(){
   ...
   async otp(){
    try{
      let response = await this.grafico_calidadService.getCausac(this.form2.value.causa, this.form.value.maquina, this.form.value.r1, this.form.value.r2, this.auth.token).toPromise();
  ...
  async call(){
    ...
    try{
      let response = await this.grafico_calidadService.getConsulc(this.form.value.maquina, this.form.value.r1, this.form.value.r2, this.auth.token).toPromise();
      if(response.code = 200){

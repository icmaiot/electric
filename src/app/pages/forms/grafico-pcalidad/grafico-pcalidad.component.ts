...
@Component({
  selector: 'app-grafico-pcalidad',
  templateUrl: './grafico-pcalidad.component.html',
  styleUrls: ['./grafico-pcalidad.component.scss'],
  encapsulation: ViewEncapsulation.None
})

export class GraficoPcalidadComponent implements OnInit {
  ...
  //Graficas
  async getGraficaPcalidad() {
    try {
      let resp = await this.graficaService.PGraficaPcalidad(this.formF.value, this.token).toPromise();
      if (resp.code == 200) {
        this.dataGraficaPcalidad = resp.response;
        this.getTurnos(this.dataGraficaPcalidad);
      }
    } catch (e) {
    }
  }
  ...
  async getTurnos(data) {
    try {
      ...
      if (resp.code == 200) {
        ...
        this.Pcalidad(this.turnos,data);
      }
    } catch (e) {
    }
  }
  ...
  //CHARTDIV4****
  Pcalidad(turnos, data) {
    let chart = am4core.create("Pcalidad", am4charts.XYChart);

    // Create axes
    let dateAxis = chart.xAxes.push(new am4charts.DateAxis());
    let valueAxis = chart.yAxes.push(new am4charts.ValueAxis());
    valueAxis.title.text = "Calidad";

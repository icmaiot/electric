...
import { GraficaIngService } from '@app/services/grafica-ing.service';
...
@Component({
  selector: 'app-grafico-ing',
  templateUrl: './grafico-ing.component.html',
  styleUrls: ['./grafico-ing.component.css'],
  encapsulation: ViewEncapsulation.None
})
export class GraficoIngComponent implements OnInit {
  form: FormGroup;
  form2: FormGroup;
  submitted = false; 
  submitted2 = false; 
  ...
  constructor(
    private zone: NgZone,
    private grafico_ingService: GraficaIngService,
  ...
  async getGraficaIng(){
    try{
      let res = await this.grafico_ingService.geti( this.auth.token ).toPromise();
      if(res.code = 200){
  ...
  async getSelect(){
    try{
      let res = await this.grafico_ingService.getMaqi( this.auth.token ).toPromise();
      if(res.code = 200){
  ...
  onSubmits2(){
   ...
   async otp(){
    try{
      let response = await this.grafico_ingService.getCausai(this.form2.value.causa, this.form.value.maquina, this.form.value.r1, this.form.value.r2, this.auth.token).toPromise();
  ...
  async call(){
    ...
    try{
      let response = await this.grafico_ingService.getConsuli(this.form.value.maquina, this.form.value.r1, this.form.value.r2, this.auth.token).toPromise();
      if(response.code = 200){
